...
export function Sidebar() {
  ...
  return (
    <aside
      className={
        `sticky top-0 z-30 h-screen shrink-0 border-r bg-linear-to-b from-[#1A3D63] to-[#0A1931] text-white ` +
        (collapsed ? "w-16" : "w-64")
      }
    >
      {/* Header / Brand */}
      <div className={`relative flex items-center ${collapsed ? "justify-center" : "justify-between"} h-14 px-3`}>
        <Link href="/project" className={`flex items-center ${collapsed ? "gap-0" : "gap-2"}`}>
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/10">
            <span className="text-lg font-bold">O</span>
          </div>
          {!collapsed && <span className="text-lg font-semibold">OneFlow</span>}
        </Link>
        {/* Toggle placed at right without affecting brand width */}
        <button
          type="button"
          onClick={() => setCollapsed((v) => !v)}
          className="absolute right-2 inline-flex h-8 w-8 items-center justify-center rounded-md text-blue-100 hover:bg-white/10"
