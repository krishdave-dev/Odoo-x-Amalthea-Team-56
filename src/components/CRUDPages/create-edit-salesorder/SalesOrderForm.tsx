...
export function SalesOrderForm({ mode = "create", initialData }: SalesOrderFormProps) {
  interface Line {
    product: string;
    qty: number;
    unit: string;
  ...
  const taxAmount = lines.reduce((s: number, l: LineState) => s + Number(l.qty) * Number(l.price) * parseTax(l.tax || "0%"), 0);
      ...
      <Card className="p-6 min-h-[520px]">
        ...
        <div className="mb-6">
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input placeholder="Select customer" className="bg-transparent border-b text-lg" />
            </div>
        ...
        <div className="mb-4">
          ...
          <Table>
            ...
            <TableBody>
              {lines.map((l: LineState, idx: number) => (
                <TableRow key={idx}>
                  <TableCell>
                    {mode === "edit" ? (
