...
interface InvoicePopupProps {
  open: boolean;
  onClose: () => void;
  onConfirm: (payload?: { lines?: LineItem[]; vendor?: string }) => void;
  lines?: LineItem[];
  onAddProduct?: () => void;
}
...
export default function InvoicePopup({ open, onClose, onConfirm, lines = [], onAddProduct }: InvoicePopupProps) {
  ...
  <Card className="relative z-10 w-[920px] max-w-full rounded-2xl border-2 border-border bg-card text-card-foreground p-6">
        <div className="mb-4 flex items-center gap-4">
          <Button className="border-dashed border-2" onClick={() => { onConfirm({ lines, vendor }); onClose(); }}>Confirm</Button>
          <Button variant="ghost" onClick={onClose}>Cancel</Button>
        </div>

  <hr className="border-border mb-4" />

        <h3 className="text-xl font-medium mb-3">Vendor Invoice</h3>
        <div className="mb-6">
          <Input value={vendor} onChange={(e) => setVendor(e.target.value)} placeholder="Select vendor" className="bg-transparent border-b text-lg" />
        </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="relative">
            <div className="absolute -right-4 -bottom-8">
              <span className="inline-flex items-center gap-2 bg-muted text-muted-foreground px-3 py-1 rounded-full text-sm">Abhi</span>
            </div>
          </div>
        </div>
      </Card>
